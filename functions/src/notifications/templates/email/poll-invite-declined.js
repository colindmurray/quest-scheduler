...
module.exports = (event, recipient) => {
  ...
  const subject = `Invite declined for "${pollTitle}"`;
  const text = `Hi ${recipientName},\n\n${inviteeName} declined your invite to join "${pollTitle}".\nView the poll: ${pollUrl}\n\nThanks!`;
  const html = `<p>Hi ${recipientName},</p><p><strong>${inviteeName}</strong> declined your invite to join "${pollTitle}".</p><p><a href="${pollUrl}">View the poll</a></p><p>Thanks!</p>`;
